...
class _CategoryViewState extends ConsumerState<CategoryView> {
  ...
  Widget _getReorderableWidget() {
    ...
    final generatedChildren = List<Widget>.generate(
      ...
      (i) {
        if (selectedSubIndex != null) {
          return _buildSubProductWidget(i, products);
        } else {
          return _buildGeneralProductWidget(i, products);
        }
      },
    );

    return selectedSubIndex != null
        ? ReorderableBuilder(
            key: Key(_gridViewKey.toString()),
            onReorder: _handleReorder,
            lockedIndices: lockedIndices,
  ...
  Widget _buildGeneralProductWidget(int i, List<ProductModel?>? products) {
    return GestureDetector(
      onTap: () {
        // final isSubProduct = selectedSubIndex != null;
        if (i == (products?.length ?? 1)) {
          final ids = widget.categories[selectedIndex].productIds;
          final name = widget.categories[selectedIndex].categaryName;
          Dialogs.singleFieldDailog(
            context,
            ids: ids,
            categoryName: name,
  ...
  // sub-product widget
  Widget _buildSubProductWidget(int i, List<ProductModel?>? products) {
    return GestureDetector(
      onTap: () {
        if (i == (products?.length ?? 1)) {
          final ids = widget.categories[selectedIndex].products![selectedSubIndex!]?.productIds;
          const name = null;
          Dialogs.singleFieldDailog(
            context,
            ids: ids,
