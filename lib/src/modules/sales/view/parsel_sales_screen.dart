...
import 'package:freelance/src/modules/sales/view/parsel_category_view.dart';

class ParselSalesView extends ConsumerWidget {
  const ParselSalesView({super.key, required this.showBills, required this.searchCtrl});
      ...
      replacement: ref.watch(categoryProvider).when(
            data: (data) => Row(
              children: [
                Flexible(
                  flex: 5,
                  child: ParselCategoryView(categories: data, searchCtrl: searchCtrl),
                ),
                const Flexible(
                  flex: 2,
                  child: BillingView(),
